...
abstract contract RateLimitDocumentation is Test {
    ...
    enum KeyType {
        ...
        AddressUint16,      // keccak256(abi.encode(bytes32, address, uint16))
        AddressEndpointId   // keccak256(abi.encode(bytes32, address, uint32)) — LayerZero endpoint IDs
    }
    ...
    EndpointEntry[]          internal _knownEndpoints;
    CentrifugeDestination[]  internal _knownCentrifugeDestinations;
    ...
    function _initializeLimitTypes() internal {
        ...
        _knownEndpoints.push(EndpointEntry(30101, "ETHEREUM"));
        _knownEndpoints.push(EndpointEntry(30106, "AVALANCHE"));
        _knownEndpoints.push(EndpointEntry(30184, "BASE"));
        _knownEndpoints.push(EndpointEntry(30102, "BNB"));
        _knownEndpoints.push(EndpointEntry(30390, "MONAD"));
        _knownEndpoints.push(EndpointEntry(30383, "PLASMA"));

        _knownCentrifugeDestinations.push(CentrifugeDestination(GroveLiquidityLayerHelpers.ETHEREUM_DESTINATION_CENTRIFUGE_ID,        "ETHEREUM"));
        _knownCentrifugeDestinations.push(CentrifugeDestination(GroveLiquidityLayerHelpers.BASE_DESTINATION_CENTRIFUGE_ID,            "BASE"));
        _knownCentrifugeDestinations.push(CentrifugeDestination(GroveLiquidityLayerHelpers.ARBITRUM_ONE_DESTINATION_CENTRIFUGE_ID,    "ARBITRUM_ONE"));
        _knownCentrifugeDestinations.push(CentrifugeDestination(GroveLiquidityLayerHelpers.PLUME_DESTINATION_CENTRIFUGE_ID,           "PLUME"));
        _knownCentrifugeDestinations.push(CentrifugeDestination(GroveLiquidityLayerHelpers.AVALANCHE_DESTINATION_CENTRIFUGE_ID,       "AVALANCHE"));
        _knownCentrifugeDestinations.push(CentrifugeDestination(GroveLiquidityLayerHelpers.BNB_SMART_CHAIN_DESTINATION_CENTRIFUGE_ID, "BNB_SMART_CHAIN"));
        ...
        _limitTypes.push(LimitType(keccak256("LIMIT_USDE_MINT"),      "USDE_MINT",      KeyType.Simple));
        _limitTypes.push(LimitType(keccak256("LIMIT_USDE_BURN"),      "USDE_BURN",      KeyType.Simple));
        _limitTypes.push(LimitType(keccak256("LIMIT_SUSDE_COOLDOWN"), "SUSDE_COOLDOWN", KeyType.Simple));
        ...
        _limitTypes.push(LimitType(keccak256("LIMIT_PSM_DEPOSIT"),      "PSM_DEPOSIT",      KeyType.Asset));
        _limitTypes.push(LimitType(keccak256("LIMIT_PSM_WITHDRAW"),     "PSM_WITHDRAW",     KeyType.Asset));
        _limitTypes.push(LimitType(keccak256("LIMIT_PENDLE_PT_REDEEM"), "PENDLE_PT_REDEEM", KeyType.Asset));
        ...
        // Centrifuge transfer key (keccak256(abi.encode(limitType, vault, uint16 destId)))
        _limitTypes.push(LimitType(GroveLiquidityLayerHelpers.LIMIT_CENTRIFUGE_TRANSFER, "CENTRIFUGE_TRANSFER", KeyType.AddressUint16));

        // LayerZero transfer key (keccak256(abi.encode(limitType, oftAddress, uint32 endpointId)))
        _limitTypes.push(LimitType(keccak256("LIMIT_LAYERZERO_TRANSFER"), "LAYERZERO_TRANSFER", KeyType.AddressEndpointId));
    }
    ...
    /**
     * @notice Print rate limits from logs
     * @param logs The logs to process for RateLimitDataSet events
     * @param chainId The chain these logs are from (for display purposes)
     */
    function _printRateLimits(Vm.Log[] memory logs, ChainId chainId) internal {
    ...
    /**
     * @notice Get accumulated logs from RecordedLogs (preserves logs for bridge relay)
     */
    function _getRecordedLogs() internal returns (Vm.Log[] memory) {
        return RecordedLogs.getLogs();
    }
    ...
    function _matchKey(bytes32 key) internal view returns (string memory) {
        ...
        for (uint256 i = 0; i < _limitTypes.length; i++) {
            ...
            if (lt.keyType == KeyType.Simple) {
                ...
                for (uint32 domain = 0; domain <= 30; domain++) {
                    if (keccak256(abi.encode(lt.hash, domain)) == key) {
                        return string.concat(lt.label, "(", vm.toString(uint256(domain)), ")");
                    }
                }
            }
            else if (lt.keyType == KeyType.AddressUint16) {
                for (uint256 j = 0; j < totalAddresses; j++) {
                    (address addr, string memory addrLabel) = _getAddressAndLabel(j);
                    for (uint256 c = 0; c < _knownCentrifugeDestinations.length; c++) {
                        if (keccak256(abi.encode(lt.hash, addr, _knownCentrifugeDestinations[c].id)) == key) {
                            return string.concat(lt.label, "(", addrLabel, ", ", _knownCentrifugeDestinations[c].name, ")");
                        }
                    }
                }
            }
            else if (lt.keyType == KeyType.AddressEndpointId) {
                for (uint256 j = 0; j < totalAddresses; j++) {
                    (address addr, string memory addrLabel) = _getAddressAndLabel(j);
                    for (uint256 e = 0; e < _knownEndpoints.length; e++) {
                        if (keccak256(abi.encode(lt.hash, addr, _knownEndpoints[e].eid)) == key) {
                            return string.concat(lt.label, "(", addrLabel, ", ", _knownEndpoints[e].name, ")");
                        }
                    }
                }
            }
